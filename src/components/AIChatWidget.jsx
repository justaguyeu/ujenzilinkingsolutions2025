...
function renderLineSegments(line, key) {
  // Split on the Ujenzi WA number in various formats
  ...
  return (
    <span key={key}>
      {segments.map((seg, si) =>
        seg.type === "wa" ? <WaButton key={si} /> : <span key={si}>{seg.val}</span>
      )}
    </span>
  );
}
